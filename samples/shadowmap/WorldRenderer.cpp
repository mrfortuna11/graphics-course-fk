...
WorldRenderer::WorldRenderer() : sceneMgr{std::make_unique<SceneManager>()} {}
...
void WorldRenderer::loadShaders() {
  etna::create_program("simple_material",
                       {SHADOWMAP_SHADERS_ROOT "simple_shadow.frag.spv",
                        SHADOWMAP_SHADERS_ROOT "simple.vert.spv"});
  ...
  // calc light matrix
  {
    const auto mProj =
        lightProps.usePerspectiveM
            ? glm::perspectiveLH_ZO(-glm::radians(packet.shadowCam.fov), 1.0f,
                                    1.0f, lightProps.lightTargetDist * 2.0f)
            : glm::orthoLH_ZO(+lightProps.radius, -lightProps.radius,
                              +lightProps.radius, -lightProps.radius, 0.0f,
...
void WorldRenderer::renderScene(vk::CommandBuffer cmd_buf,
  ...
  for (std::size_t instIdx = 0; instIdx < instanceMeshes.size(); ++instIdx) {
    ...
    for (std::size_t j = 0; j < meshes[meshIdx].relemCount; ++j) {
      ...
      cmd_buf.drawIndexed(relem.indexCount, 1, relem.indexOffset,
                          relem.vertexOffset, 0);
    }
  }
}
...
void WorldRenderer::renderWorld(vk::CommandBuffer cmd_buf,
...

  {
    ...
    auto set = etna::create_descriptor_set(
        simpleMaterialInfo.getDescriptorLayoutId(0), cmd_buf,
        {etna::Binding{0, constants.genBinding()},
         etna::Binding{1, shadowMap.genBinding(
                              defaultSampler.get(),
    ...
    cmd_buf.bindPipeline(vk::PipelineBindPoint::eGraphics,
                         basicForwardPipeline.getVkPipeline());
    cmd_buf.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
                               basicForwardPipeline.getVkPipelineLayout(), 0,
                               {set.getVkSet()}, {});
  ...
  if (drawDebugFSQuad)
    quadRenderer->render(cmd_buf, target_image, target_image_view, shadowMap,
                         defaultSampler);
}
...
void WorldRenderer::drawGui() {
  ...
  ImGui::ColorEdit3("Meshes base color", color,
                    ImGuiColorEditFlags_PickerHueWheel |
                        ImGuiColorEditFlags_NoInputs);
  uniformParams.baseColor = {color[0], color[1], color[2]};

  float pos[3]{uniformParams.lightPos.x, uniformParams.lightPos.y,
               uniformParams.lightPos.z};
