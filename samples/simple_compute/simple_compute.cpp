...
SimpleCompute::SimpleCompute() : length{16} {}

void SimpleCompute::setup() {
  etna::create_program("simple_compute",
                       {SIMPLE_COMPUTE_SHADERS_ROOT "simple.comp.spv"});
  ...
  pipeline =
      context->getPipelineManager().createComputePipeline("simple_compute", {});
}
...
void SimpleCompute::buildCommandBuffer(vk::CommandBuffer cmd_buf) {
  ...
  auto set = etna::create_descriptor_set(
      simpleComputeInfo.getDescriptorLayoutId(0), cmd_buf,
      {
  ...
  cmd_buf.bindDescriptorSets(vk::PipelineBindPoint::eCompute,
                             pipeline.getVkPipelineLayout(), 0, 1, &vkSet, 0,
                             nullptr);

  cmd_buf.pushConstants(pipeline.getVkPipelineLayout(),
                        vk::ShaderStageFlagBits::eCompute, 0, sizeof(length),
                        &length);
