...
App::App() : resolution{1280, 720}, useVsync{true} {
  {
    ...
    etna::initialize(
        etna::InitParams{.applicationName = "Local Shadertoy",
  ...
  etna::create_program("toy", {LOCAL_SHADERTOY2_SHADERS_ROOT "toy.frag.spv",
                               LOCAL_SHADERTOY2_SHADERS_ROOT "toy.vert.spv"});
  etna::create_program("proc",
  ...
  auto generateTexMipLevels = [this](etna::Image &tex, uint32_t w, uint32_t h,
                                     uint32_t mip_count, uint32_t layer_count) {
    ...
    ETNA_CHECK_VK_RESULT(cmdBuf.begin(vk::CommandBufferBeginInfo{}));
    {
      ...
      for (uint32_t level = 1; level < mip_count; ++level) {
        ...
        blit.dstOffsets[1] = vk::Offset3D{(int32_t)std::max(w / 2, 1u),
                                          (int32_t)std::max(h / 2, 1u), 1};

        w = std::max(w / 2, 1u);
        h = std::max(h / 2, 1u);

        cmdBuf.blitImage(tex.get(), vk::ImageLayout::eGeneral, tex.get(),
                         vk::ImageLayout::eGeneral, {blit},
                         vk::Filter::eLinear);
      }
...

  {
    ...
    unsigned char *texData =
        stbi_load(GRAPHICS_COURSE_RESOURCES_ROOT "/textures/tree.png", &texW,
                  &texH, &texChannels, 0);
    ETNA_VERIFY(texData);

    ETNA_VERIFYF(texChannels == 3 || texChannels == 4, "Invalid channels={}",
                 texChannels);
    std::vector<std::byte> imageData{(size_t)(texH * texW * 4)};
    size_t dstId = 0;
    for (auto *src = texData; src != texData + texW * texH * texChannels;
         ++src) {
    ...
    transferHelper->uploadImage(*oneShotCommands, sourceTexture, 0, 0,
                                imageData);
  ...
  defaultSampler =
      etna::Sampler{etna::Sampler::CreateInfo{.name = "default_sampler"}};
  detailSampler = etna::Sampler{
      etna::Sampler::CreateInfo{.filter = vk::Filter::eLinear,
  ...
  shadertoyPipeline = pipelineManager.createGraphicsPipeline(
      "toy", etna::GraphicsPipeline::CreateInfo{
  ...
  proceduralPipeline = pipelineManager.createGraphicsPipeline(
      "proc", etna::GraphicsPipeline::CreateInfo{
                  .fragmentShaderOutput = {.colorAttachmentFormats = {
                                               vkWindow->getCurrentFormat()}}});
}
...
void App::drawFrame() {
  ...
  if (nextSwapchainImage) {
    auto [backbuffer, backbufferView, backbufferAvailableSem,
          backbufferReadyForPresent] = *nextSwapchainImage;
    ...
    {
      {
        ...
        auto set = etna::create_descriptor_set(
            procProgInfo.getDescriptorLayoutId(0), currentCmdBuf,
        ...
        currentCmdBuf.bindDescriptorSets(
            ...
            proceduralPipeline.getVkPipelineLayout(), 0, {set.getVkSet()}, {});
...

      {
        ...
        auto set = etna::create_descriptor_set(
            toyProgInfo.getDescriptorLayoutId(0), currentCmdBuf,
            {etna::Binding{7, uniformParams.genBinding()}});
        auto imgSet = etna::create_descriptor_set(
            toyProgInfo.getDescriptorLayoutId(1), currentCmdBuf,
            {etna::Binding{0, proceduralImage.genBinding(
                                  defaultSampler.get(),
                                  vk::ImageLayout::eShaderReadOnlyOptimal)},
             etna::Binding{1, sourceTexture.genBinding(
                                  detailSampler.get(),
                                  vk::ImageLayout::eShaderReadOnlyOptimal)}});
        ...
        currentCmdBuf.bindDescriptorSets(
            vk::PipelineBindPoint::eGraphics,
            shadertoyPipeline.getVkPipelineLayout(), 0,
      ...
      etna::set_state(currentCmdBuf, mainImage.get(),
...

      {
        ...
        currentCmdBuf.blitImage(
            mainImage.get(), vk::ImageLayout::eTransferSrcOptimal, backbuffer,
